...
unction createNadplataKredytuGroup() {
    const group = document.createElement("div");
    group.classList.add("variable-input-group");
    group.innerHTML = `
        <div class="box-period-start">
            <label class="form-label" for="nadplata-cykl-start">W</label>
            <input type="number" class="form-control variable-cykl-start" id="nadplata-cykl-start" min="1" value="1">
            <span class="unit-period">miesiącu</span>
            <input type="range" class="form-range variable-cykl-start-range" min="1" max="360" value="1" step="1">
        </div>
        <div class="box-rate">
            <label class="form-label" for="nadplata-rate">Kwota nadpłaty</label>
            <input type="number" class="form-control variable-rate" id="nadplata-rate" min="100" value="100">
            <span class="unit-rate">PLN</span>
            <input type="range" class="form-range variable-rate-range" min="100" max="500000" value="100" step="100">
        </div>
        <div class="box-select">
            <label class="form-label" for="nadplata-type">Typ nadpłaty</label>
            <select class="form-select nadplata-type-select" id="nadplata-type">
                <option value="Jednorazowa">Jednorazowa</option>
            </select>
        </div>
        <div class="box-select">
            <label class="form-label" for="nadplata-effect">Efekt nadpłaty</label>
            <select class="form-select nadplata-effect-select" id="nadplata-effect">
...
function initializeNadplataKredytu() {
    const wrapper = document.getElementById("nadplataKredytuWrapper");
    if (!wrapper) return;

    // Dodaj pierwszą grupę nadpłaty, jeśli jeszcze nie istnieje
    ...
    updateAllOverpaymentLimits("all");
...
function calculateRemainingCapital(loanAmount, interestRate, totalMonths, paymentType, overpayments, targetMonth) {
    ...
    if (targetMonth <= 1) {
        ...
        if (paymentType === "rowne") {
            ...
            const rataCalkowita = (loanAmount * (q ** totalMonths) * (q - 1)) / ((q ** totalMonths) - 1);
            rataKapitalowa = rataCalkowita - odsetki;
        } else {
            rataKapitalowa = loanAmount / totalMonths;
        }
    ...
    for (let month = 1; month <= targetMonth && remainingCapital > 0; month++) {
        ...
        if (paymentType === "rowne") {
            const remainingMonthsForCalc = remainingMonths - (month - 1);
            const q = 1 + monthlyRate;
            let rataCalkowita = remainingCapital * (q ** remainingMonthsForCalc) * (q - 1) / ((q ** remainingMonthsForCalc) - 1);
        ...
        if (applyOverpayment && remainingCapital > 0) {
            ...
            if (overpaymentEffect === "Skróć okres") {
                if (remainingCapital > 0) {
                    const newRemainingMonths = Math.ceil(remainingCapital / (loanAmount / totalMonths));
                    remainingMonths = month + newRemainingMonths - 1;
                } else {
...
function updateOverpaymentLimit(input, range, group, preserveValue = true, triggerSource = "all") {
    ...
    let adjustedPeriod = periodStart === 1 ? 1 : Math.max(1, periodStart - 1);
    ...
    if (triggerSource === "all" || triggerSource === "rate") {
        let maxAllowed = Math.max(100, remainingCapital);
        rateInput.max = Math.floor(maxAllowed);
        rateRange.max = Math.floor(maxAllowed);
    }

    if (!preserveValue || triggerSource === "all") {
        let rateValue = Math.min(parseInt(rateInput.value) || 100, parseInt(rateInput.max) || 100);
        rateInput.value = rateValue;
        range.value = rateValue;
    } else {
        let rateValue = parseInt(rateInput.value) || 100;
        if (rateValue > parseInt(rateInput.max)) {
            rateValue = parseInt(rateInput.max);
            rateInput.value = rateValue;
            range.value = rateValue;
        }
    }
...
function updateNadplataKredytuRemoveButtons() {
    ...
    if (loanDetails) {
        const lastGroup = groups[groups.length - 1];
        const remainingCapital = loanDetails.harmonogram[loanDetails.harmonogram.length - 1]?.kapitalDoSplaty || loanAmount; // Ustawiamy domyślną wartość na loanAmount, jeśli harmonogram jest pusty
        const remainingMonths = loanDetails.pozostaleRaty || totalMonths;
        const lastPeriodStart = parseInt(lastGroup.querySelector(".variable-cykl-start")?.value) || 1;

        if (remainingCapital <= 0 || lastPeriodStart >= remainingMonths) {
...
function initializeNadplataKredytuGroup(group) {
    ...
    const updatePeriodBox = () => {
        ...
        periodStartInput.value = Math.max(minValue, parseInt(periodStartInput.value) || minValue);
        periodStartRange.value = periodStartInput.value;

        updateOverpaymentLimit(rateInput, rateRange, group, true, "period");
        updateNadplataKredytuRemoveButtons();
            ...
            if (input.classList.contains("variable-cykl-start")) {
                const debouncedUpdate = debounce(() => {
                    if (!state.isUpdating) {
                        const rateInput = group.querySelector(".variable-rate");
                        const rateRange = group.querySelector(".variable-rate-range");
                        if (rateInput && rateRange) updateOverpaymentLimit(rateInput, rateRange, group, true, "period");
                        updateRatesArray("nadplata");
                        updateNadplataKredytuRemoveButtons();
                        updateLoanDetails();
                    }
                ...
                const debouncedUpdate = debounce(() => {
                    if (!state.isUpdating) {
                        updateOverpaymentLimit(input, range, group, false, "rate");
                        updateRatesArray("nadplata");
                        updateNadplataKredytuRemoveButtons();
                        updateLoanDetails();
                    }
...
document.addEventListener("DOMContentLoaded", () => {
    initializeNadplataKredytu();

    // Słuchacz dla checkboxa nadpłaty
    const nadplataCheckbox = document.getElementById("nadplataKredytuBtn");
    const nadplataInputs = document.getElementById("nadplataKredytuInputs");
    if (nadplataCheckbox && nadplataInputs) {
        nadplataCheckbox.addEventListener("change", () => {
            nadplataInputs.classList.toggle("active", nadplataCheckbox.checked);
            if (nadplataCheckbox.checked && nadplataInputs.children.length === 0) {
                initializeNadplataKredytu();
            }
            updateNadplataKredytuRemoveButtons();
        });
...
function calculateInstallment(kwota, iloscRat, pozostalyKapital, currentOprocentowanie, nadplata, activeOverpayment, rodzajRat, miesiac) {
    ...
    if (rodzajRat === "rowne") {
        ...
        if (remainingMonths <= 0) remainingMonths = 1; // Zapobiegamy dzieleniu przez 0

        // Oblicz ratę całkowitą na podstawie pozostałego kapitału i pozostałego okresu
        if (activeOverpayment && activeOverpayment.effect === "Zmniejsz ratę" && nadplata > 0) {
            // Po nadpłacie przeliczamy ratę na nowo
            rataCalkowita = pozostalyKapital * (q ** remainingMonths) * (q - 1) / ((q ** remainingMonths) - 1);
        } else {
            // Standardowa rata równa dla całego kredytu
        ...
        // Uwzględnij nadpłatę w przypadku "Zmniejsz ratę"
        if (activeOverpayment && activeOverpayment.effect === "Zmniejsz ratę" && nadplata > 0) {
...
function calculateLoan(kwota, oprocentowanie, iloscRat, rodzajRat, prowizja, prowizjaJednostka, variableRates = [], overpaymentRates = []) {
    try {
        ...
        let jednostka = prowizjaJednostka; // Używamy przekazanej wartości
...
function updateLoanDetails() {
    ...
    const prowizja = parseFloat(elements.prowizja?.value) || 0;
    const prowizjaJednostka = elements.jednostkaProwizji?.value || "procent"; // Poprawione
